...
module.exports = function(resultSender, tag, knownUrls, journal, max) {
  ...
  const mergeItemMapStream = new MergeItemMapStream(journal.itemMap)
  const limitStream = new LimitStream(fetchItemListStream, mergeItemMapStream, max)

  const itemStream = pageStream
    .pipe(fetchItemListStream)
    .pipe(mergeItemMapStream)
    .pipe(new FetchItemStream(journal.ignoreItems))
