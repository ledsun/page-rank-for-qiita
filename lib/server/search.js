...
module.exports = function search(tag, ws, journal) {
  const pageStream = new PageStream(tag)
  const recentItemsStream = new RecentItemsStream(pageStream)
  const fetchPageRankStream = new FetchPageRankStream(journal.pageRanks)
  ...
  // クライアントに結果送信
  itemStream.pipe(new WebSocketStream(ws, 'item', tag))
