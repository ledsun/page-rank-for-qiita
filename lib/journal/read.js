...
module.exports = function(file) {
  ...
  const s = fs.createReadStream(file, {
    ...
    .pipe(es.mapSync((line) => {
        ...
        const [url, title, countStr, content_length, tags, created_at, updated_at] = line.split('\t')
        // 文字列で取れるので数値に型変換
        ...
        if (url) {
          ...
          if (url) {
            itemMap.set(url, title, count, content_length, calculatePageRank(count, content_length), tagArray, updated_at)
          }
